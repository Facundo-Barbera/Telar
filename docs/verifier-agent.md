...
- **Status (2026-07-09): SHIPPED (core)** — `814bc01` (`distill.ts` + `spec-lint.ts` + `playwrightConfigTemplate`) + `136cc4c`. Acceptance 1–2 proven by unit tests: `distillSpec` emits `getByRole`/`getByLabel`/`getByText` locators only (unsafe raw selectors discarded by construction — `SAFE_LOCATOR` full-match) and `spec-lint` flags an injected `page.locator('.foo')`; 146 tests green, including a CR/CRLF string-literal escaping regression the adversarial verify pass caught before commit. Acceptance 3–4 (live headless green-run in <2s, flaky-kickback) **deferred → task #11** (need a running app + `verified-loop.ts` + the real `spec-lint` manifest gate wired).
...
- **Status (2026-07-09): SHIPPED (core)** — `ff9121e`. `VerifierReport.designFindings[]` (severity + category + screenshot) + `ProjectManifest.designRules` (read best-effort in `runVerification`, injected for the agent to cite); a design-critique pass appended to the Verifier prompt (functional instructions unchanged); rendered as a separate "Design findings" section in the report card. **Non-gating is proven, not asserted**: `classify()`/`decide()` are untouched and structurally cannot see `designFindings`, so a blocker-severity finding alongside all-pass criteria still classifies `pass` (locked by `design-findings.test.ts`; 153 tests green). Live acceptance (the ugly-but-functional demo + a sample design-rules doc) **deferred → task #13**.
