...
2026-08-12 nightly). It is the closest existing thing to what Telar Telar wants
...
are the contract's words, and every Telar surface reads them. What the sweep
...
Telar replaces rather than carries forward.
...
- **`ultra:run-anchor`**, the kind id retired INV-10 used to pin, becomes
  `warp:run` **when the Telar warp surface is built**, not before — there is
  nothing to re-pin it against yet.
- **Persisted runs** under `TELAR_HOME/ultra`: abandoned deliberately. Telar
  writes only beneath `TELAR_HOME/engine` and never reads legacy run history.

**What a Telar Warp still has to do**, and has not: run a warp script inside the
...
`apps/web` renders a real timeline — collapsed tool cards with output,
...
3. **Stage 1 is contract-first.** Write the full v2 type surface in
   ...
   contract is reviewable before behaviour changes, and `apps/web` can
...
4. **zod in `engine-client`, types derived with `z.infer`.** Not `effect/Schema`
   ...
   `apps/web/lib/engine/journal.ts` hand-checked `typeof event.data.text
