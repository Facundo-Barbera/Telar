# Deep Agents JS: installed-package audit — 2026-09-15

**What this is.** The companion audit to
[`528-agent-lab-langgraph-vs-deep-agents-2026-09-15.md`](528-agent-lab-langgraph-vs-deep-agents-2026-09-15.md):
what the `deepagents` package actually exports, injects and assumes, read off
the installed 1.13.4 rather than off its documentation. #528 asked for exactly
this — "consult the current official JS docs AND the installed package's
exported API; docs mix Python. No framework choice by assertion" — and §10 is
the result: eight verified JS-versus-Python mismatches.

**When it was measured.** 2026-09-15, produced under
`prototypes/agent-lab/deepagents-audit/` as part of draft PR #529, which was
never merged. Its runnable probes are not in this repository.

**The decision it fed, and where that decision is now.** Telar took LangGraph.js
and did not adopt Deep Agents (#531); the Agent built on that choice ships in
`apps/engine/src/agent/`. This audit is the record of what the rejected option
would have brought with it.

**The numbers below are the prototype's own and were not re-run.** The 8
injected tools, the footprint figures, the zod finding and the API listings are
that session's measurements against `deepagents@1.13.4` on 2026-09-15, not a
current reading of the package.

The audit follows unedited.
