...
- OpenCode: real SDK fixture tests cover streaming, lost admission acknowledgment, permissions, questions, admission-time Stop, unknown admission, and terminal provider failure. `smoke-opencode.ts` ran the actual 1.18.30 CLI against an isolated local model fixture and completed a real turn without credentials or paid calls. A second real-process smoke verifies that two servers in the same checkout do not share runtime MCP registrations, do not write a project config file, and can shut down independently.
...
- Packaged Electron smoke passed at `da1c110c`: production SQLite startup, an embedded worker registration, bundled Playwright MCP and agent SDK, and the standalone web app. Final CI is recorded in the PR. Local package output is not an installation or nightly publication.
