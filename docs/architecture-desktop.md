...
The bulk of its build-time complexity is **build-time bundling glue**, because the app is two processes and neither ships as its source: `build-app.sh` bundles the engine into a single `engine.mjs`, lays the Agent SDK's JavaScript beside it (kept external so the CLI version pairing stays readable), and materializes the `@playwright/mcp` CLI closure the engine spawns. A `--smoke` mode boots the real engine, waits for `/v2/health`, then boots the real server and `waitForServer`s it, asserting both bundles are present, exiting 0/1 without ever creating a window; this is the fail-closed gate `scripts/build-desktop.sh` uses after every package build.

**The Agent SDK's ~272MB native CLI binary is not bundled.** It once was, and removing it is what makes Telar's dependency on the user's own Claude Code install honest rather than merely true: the engine resolves that install and refuses the turn with an actionable message when it is missing or speaks a different control protocol (`apps/engine/src/cli-resolution.ts`).
...
| Hosted app | The `apps/web` Next.js 16 (canary) standalone build (`server.js`), forked as a child process — not embedded via any Electron-Next integration library | [build-app.sh](../apps/desktop/build-app.sh) |
...
| [build-app.sh](../apps/desktop/build-app.sh) | Builds the `apps/web` Next standalone output into `.next-desktop/`, then materializes the two bundled binaries | 75 |
...
|---|---|---|
| `build:web` | `bash ./build-app.sh` | Builds the standalone Next server + bundles binaries |
...
├── build-app.sh                  # builds .next-desktop standalone + bundles binaries
...
2. **`--smoke` mode** (`runSmoke()` in [main.js](../apps/desktop/main.js)) — the primary automated correctness gate for a *packaged* build. It is invoked directly against the packed `.app` binary (not via `electron .`) by both [install-app.sh](../apps/desktop/install-app.sh) and [../scripts/build-desktop.sh](../scripts/build-desktop.sh), and exercises: the real forked server boots and answers HTTP; the real forked ENGINE boots and answers `/v2/health`; the bundled `@playwright/mcp` CLI exists on disk; the bundled Agent SDK entry exists on disk. It is fail-closed only in packaged mode (`app.isPackaged`) — in a dev-repo checkout the bundle checks degrade to best-effort logging, since dev-repo resolves both through the runtime walk-up instead. **It deliberately does NOT assert that Claude Code is installed**: the app no longer ships one, whether a machine has one is not a property of the artefact, and asserting it here would fail every release build on a CI runner that has no reason to have one. This — plus the e2e harness below — needs a real macOS Electron and stays on the mac release path, not the ubuntu `verify.yml` job.
...
|---|---|---|---|
| **`apps/web` standalone server** | desktop → web (spawns) | `child_process.fork()` of `server.js`, built by `next build` with `NEXT_OUTPUT=standalone NEXT_DIST_DIR=.next-desktop` | [main.js](../apps/desktop/main.js) `startServer()`, [build-app.sh](../apps/desktop/build-app.sh) |
...
| **Agent SDK (JavaScript)** | build-time materialization, runtime resolution | `build-app.sh` copies the real (symlink-dereferenced) `@anthropic-ai/claude-agent-sdk` package (3.9MB, no native binary) into `engine/dist/node_modules`, beside the engine bundle that keeps it external. Node resolves it by walking up from `engine.mjs`. | [build-app.sh](../apps/desktop/build-app.sh), [main.js](../apps/desktop/main.js) `bundledAgentSdkEntry()` |
| **Claude Code CLI** | runtime resolution only — **not bundled** | The engine resolves the user's own install (`CLAUDE_CODE_EXECUTABLE` → `~/.local/bin` → brew prefixes → `PATH` last) and passes it as `pathToClaudeCodeExecutable`, refusing the turn with an actionable message when there is none. Removing the ~272MB bundled binary is what makes that dependency honest rather than merely true. | [cli-resolution.ts](../apps/engine/src/cli-resolution.ts) |
...
| **`scripts/build-desktop.sh` (repo root)** | orchestrates apps/desktop | The only reproducible pipeline invoking `build-app.sh` and `electron-builder` together: pristine `git worktree` snapshot of an origin ref → `bun install --frozen-lockfile` → `build-app.sh` → stamp `build-info.json` → `electron-builder --dir` (or `zip,dmg` for release builds) → atomic swap into `--out` (default `apps/desktop/release/from-origin`) → `--smoke` gate → optional `--publish-r2` upload. Never reads the live working tree as a source. | [../scripts/build-desktop.sh](../scripts/build-desktop.sh) |
...
- **Build-time materialization must use `cp -RL`, not symlinks.** Both `@playwright/mcp` and the Agent SDK are copied with symlink dereferencing (`cp -RL`) into their slots — packaged `.app` bundles cannot rely on bun's `.bun`-store symlink indirection since the store itself isn't shipped.
- **`asar: true`, but only the shell's own files go through it.** `files` in `package.json`'s `build` block lists exactly the shell's own modules — and must list every one `main.js` requires, which `packaging.test.js` derives and checks — while every other runtime dependency (the entire Next server, the engine bundle, the Agent SDK, the MCP CLI) ships via `extraResources`, which sits outside the asar archive under `Contents/Resources`, because native binaries and large dependency trees generally can't run correctly from inside an asar archive.
