...
import { GET as projectsGet, POST as projectsPost } from "@/app/api/vnext/projects/route";
import { GET as eventsGet } from "@/app/api/vnext/sessions/[sessionId]/events/route";
import { POST as discardPost } from "@/app/api/vnext/sessions/[sessionId]/turns/[runId]/discard/route";
...
describe("vNext route adapters", () => {
  ...
  test("rejects malformed command input before it reaches the engine", async () => {
    const response = await projectsPost(new Request("http://telar.local/api/vnext/projects", { method: "POST", body: "not json" }));
    expect(response.status).toBe(400);
    expect((await response.json()).error.code).toBe("invalid_request");
  });

  test("validates a journal cursor in the thin adapter", async () => {
    const response = await eventsGet(new Request("http://telar.local/api/vnext/sessions/session_a/events?after=not-a-cursor"), { params: Promise.resolve({ sessionId: "session_a" }) });
    expect(response.status).toBe(400);
    expect((await response.json()).error.code).toBe("invalid_request");
  });
  ...
  test("proxies an explicit discard decision to the authenticated engine without submitting a replay", async () => {
    ...
    const response = await discardPost(new Request("http://telar.local/api/vnext/sessions/session_one/turns/uncertain_run/discard", { method: "POST" }), {
