    ...
    return (
      ...
      >
        ...
        <text
          ...
          className={`text-[10px] ${emphasis ? "fill-foreground font-medium" : "fill-foreground/80"}`}
        >
            ...
            <svg width={width} height={height} role="img" aria-label="Loom braid">
              ...
              {braid.strands.map((strand, index) => {
                ...
                return (
                  <g key={strand.slug}>
                    ...
                    <text x={x + 14} y={y(first.row) - 14} className="fill-muted-foreground text-[10px] font-medium">
              ...
              {[...braid.spine, ...braid.strands.flatMap((s) => s.nodes)]
                ...
                .map((node, i, all) => {
                  ...
                  return (
                    <text key={`t-${node.id}`} x={8} y={y(node.row) + 3} className="fill-muted-foreground text-[9px] tabular-nums">
                      {label}
                    </text>
                  );
                })}
