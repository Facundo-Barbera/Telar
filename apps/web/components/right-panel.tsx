    ...
    <div>
      <PanelRow tone={taskTone(state)} className="p-0 pl-0">
        ...
        >
          ...
          {tokens > 0 && <span className="shrink-0 font-mono text-3xs text-muted-foreground tabular-nums">{figure(tokens)}</span>}
          <span className={cn("shrink-0 font-mono text-3xs", state === "failed" ? "text-destructive" : "text-muted-foreground")}>
            {TASK_STATE[state]}
          </span>
      ...
      {open && (
        <div className="border-border/60 border-l pl-1">
          ...
          {group.run?.failure && <p className="px-4 py-2 text-2xs text-destructive">{group.run.failure}</p>}
        </div>
      )}
    </div>
  );
