...
function LobbyRow({
  ...
  const row = (
    ...
    >
      ...
      <SessionLiveMark live={subject.sessionLive} />
      {subject.needsYou > 0 && (
        <span className="shrink-0 text-[11px] tabular-nums text-muted-foreground">{subject.needsYou}</span>
      )}
    </button>
  );
  return (
...
function LobbyNode({
  ...
  const containerRow = (
    ...
    >
      ...
      {ceiling && <span className="shrink-0 text-[11px] text-muted-foreground/50">ceiling: {ceiling}</span>}
      {collapsed ? (
