      ...
      {showing && (
        ...
        >
          {KINDS.map(({ kind, header }) => {
            ...
            return (
              <div key={kind}>
                <p className="px-2 pt-1.5 pb-0.5 text-3xs font-semibold tracking-[0.12em] text-muted-foreground/70 uppercase">
                  {header}
                </p>
                <ul>
                  {group.map((hit) => (
                    <li key={`${hit.kind}:${hit.id}`}>
                      ...
                      >
                        <span className="flex min-w-0 items-center gap-1.5">
                          ...
                          {hit.closed && <span className="shrink-0 text-3xs text-muted-foreground/50">· over</span>}
                          {hit.subject && (
                            <span className="shrink-0 font-mono text-3xs text-muted-foreground/60">{hit.subject}</span>
                          )}
                        </span>
                        <span
                          className={cn(
                            "block min-w-0 truncate text-2xs leading-relaxed",
