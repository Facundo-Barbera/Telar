...
function Fact({ fact, scope, onChanged }: { fact: SpoolMemoryFact; scope?: string; onChanged: () => void }) {
  ...
  return (
    <li className="group/fact rounded-md px-2 py-1.5 transition-colors hover:bg-muted/50">
      ...
      <div className="mt-1 flex flex-wrap items-center gap-x-2 gap-y-1 text-[10px] text-muted-foreground/60">
      ...
      {error && <p className="mt-1 text-[10px] leading-relaxed text-destructive">{error}</p>}
    </li>
  );
}
      ...
      <div className="min-h-0 flex-1 overflow-y-auto p-1.5">
        ...
        {memory && groups.length === 0 && (
          <div className="flex flex-col items-center gap-2 px-4 py-10 text-center">
            ...
            <p className="max-w-52 text-[11px] leading-relaxed text-muted-foreground/50">
        ...
        {groups.map((group) => (
          <section key={group.key} className="mb-2">
            <p className="px-2 pt-1 pb-1 text-[10px] font-medium tracking-wider text-muted-foreground/70 uppercase">
              {group.label}
            </p>
