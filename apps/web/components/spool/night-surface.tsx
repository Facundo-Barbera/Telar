...
function Label({ children }: { children: React.ReactNode }) {
  return (
    <p className="px-3 pt-3 pb-1.5 text-3xs font-medium tracking-wider text-muted-foreground/70 uppercase">
      {children}
    </p>
  );
}
  ...
  return (
    <div className="flex h-full flex-col">
      <div className="min-h-0 flex-1 overflow-y-auto">
                ...
                <li key={job.id} className="rounded-xl bg-card p-2.5 shadow-1 ring-1 ring-foreground/10">
                  ...
                  <ul className="mt-1.5 space-y-1">
                    {job.openQuestions?.map((question) => (
                      <li key={question} className="flex items-start gap-2 text-2xs leading-relaxed text-muted-foreground">
        ...
        {night && (
          <>
                ...
                const body = (
                  <>
                    ...
                    <span className="min-w-0 flex-1">
                      ...
                      {job.note && (
                        <span className="mt-0.5 block text-2xs leading-relaxed text-muted-foreground">{job.note}</span>
                      )}
                    </span>
                  </>
                );
            ...
            {night.stop && (
              <p className={cn("px-3 pt-2 text-2xs leading-relaxed", STOP_TONE[night.stop.reason] ?? "text-muted-foreground")}>
                {night.stop.note}
              </p>
            )}
            {night.usage?.costUsd !== undefined && (
              ...
              <p className="px-3 pt-1 font-mono text-3xs text-muted-foreground/60 tabular-nums">
      ...
      <div className="shrink-0 space-y-2 border-t border-border p-2">
        ...
        {refusal ? (
          <p className="rounded-lg bg-muted/60 px-2 py-1.5 text-2xs leading-relaxed text-muted-foreground">{refusal}</p>
        ) : (
