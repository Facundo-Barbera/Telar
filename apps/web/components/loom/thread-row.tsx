...
export function ThreadStatusSlot({ live }: { live: ThreadLiveness | null }) {
  if (!live?.activity) {
    return <span className="shrink-0 text-2xs text-muted-foreground">unreachable</span>;
  }
  ...
  return (
    <span className={`inline-flex shrink-0 items-center gap-1 text-2xs font-medium ${ACTIVITY_TONE[badge.tone]}`}>
      {badge.ticking ? (
...
export function LoomStateBadge({ state }: { state: LoomDisplayStateName }) {
  ...
  return (
    <Badge variant="outline" className={`font-mono text-3xs uppercase tracking-wide ${tone[state]}`}>
      {state}
    </Badge>
  );
}
...
export function TierBadge({ tier }: { tier?: string }) {
  return tier ? (
    ...
    <Badge variant="outline" className="font-mono text-3xs uppercase text-warning" title="No executable tier backs this contract">
      no tier
    </Badge>
  );
}
...
export function VerificationBadge({
  ...
  return (
    <Badge variant="outline" className={`font-mono text-3xs ${verification.ok ? "text-success" : "text-destructive"}`}>
