...
export default function DashboardPage() {
  ...
  const load = useCallback(() => {
    ...
    fetch("/api/usage")
      .then((r) => (r.ok ? r.json() : null))
      .then((d) => d?.ledger?.session && setUsage(d.ledger.session))
      .catch(() => {});
  }, []);
  ...
  // The workspace is Telar's front door now. The dashboard remains available
  // as an explicit Overview destination, but launching the app at `/` drops a
  // returning user directly into a fresh session for their first registered
  // project — the composer then owns switching to another checkout.
  useEffect(() => {
    if (!projectsLoaded || !sessionsLoaded || projects.length === 0) return;
    if (new URLSearchParams(window.location.search).has("overview")) return;
    const knownProjects = new Set(projects.map(({ entry }) => entry.name));
    const project = [...sessions]
      .sort((a, b) => b.updatedAt - a.updatedAt)
      .find((session) => session.project && knownProjects.has(session.project))?.project
      ?? projects[0]?.entry.name;
    if (project) router.replace(`/projects/${encodeURIComponent(project)}/sessions/new`);
  }, [projects, projectsLoaded, router, sessions, sessionsLoaded]);
            ...
            <>
              ...
              <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-5">
                ...
                <StatTile
                  ...
                  sub={`${new Intl.NumberFormat().format((usage?.inputTokens ?? 0) + (usage?.outputTokens ?? 0))} tokens recorded`}
