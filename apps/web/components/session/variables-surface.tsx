          ...
          {vars.map((row) => (
            <div key={row.name} className="border-b border-border/60">
              <button type="button" onClick={() => void inspect(row.name)} className={cn("flex w-full items-center gap-2 px-3 py-1.5 text-left outline-none hover:bg-muted/40 focus-visible:ring-2 focus-visible:ring-ring", open === row.name && "bg-muted/40")}>
                <span className="min-w-0 flex-1 truncate font-mono text-2xs text-foreground">{row.name}</span>
                <span className="shrink-0 truncate font-mono text-3xs text-muted-foreground">{row.type}</span>
                <span className="w-24 shrink-0 text-right font-mono text-3xs text-muted-foreground tabular-nums">
                  {row.shape ? row.shape.join("×") : row.len !== undefined ? `len ${row.len}` : ""}
                </span>
                <span className="w-16 shrink-0 text-right font-mono text-3xs text-muted-foreground tabular-nums">{row.sizeBytes ? humanBytes(row.sizeBytes) : ""}</span>
              </button>
              {open === row.name && (
                <pre className="m-0 max-h-72 overflow-auto whitespace-pre-wrap border-t border-border/40 bg-muted/20 px-3 py-2 font-mono text-3xs leading-[1.5]">
