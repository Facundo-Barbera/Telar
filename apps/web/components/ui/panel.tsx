...
export function PanelHeader({
  ...
  return (
    <div
      className={cn(
        "flex shrink-0 items-center gap-1.5 border-b border-border px-3 py-2 font-mono text-[10px] tracking-[0.08em] text-muted-foreground uppercase",
        className,
      )}
      {...props}
    >
...
export function PanelDivider({ label }: { label: string }) {
  return (
    <div className="flex items-center gap-2 px-3 py-1.5">
      ...
      <span className="font-mono text-[9px] tracking-[0.08em] text-muted-foreground uppercase">{label}</span>
      <span className="h-px flex-1 bg-border" />
    </div>
  );
}
