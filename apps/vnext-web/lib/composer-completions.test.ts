...
const FILES = [
  ...
  "apps/vnext-web/components/composer.tsx",
  "apps/vnext-web/lib/drag-reference.ts",
...
describe("ranking paths", () => {
  ...
  test("an accepted path inserts EXACTLY what dragging the same row would", () => {
    ...
    expect(file?.action).toEqual({ type: "insert", text: fileReference("apps/vnext-web/lib/drag-reference.ts").text });
