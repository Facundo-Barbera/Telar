import { requestObject, vnextEngine, vnextErrorResponse } from "@/lib/vnext/engine-server";
...
export async function GET() {
  try {
    return Response.json(await (await vnextEngine()).inboxPolicy());
  } catch (error) {
...
export async function PATCH(request: Request) {
  try {
    ...
    return Response.json(
      await (await vnextEngine()).setInboxPolicy({
