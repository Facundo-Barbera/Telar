...
export function useUltraWake(sessionId: string | null) {
  ...
  // Wakes are durable, so their first check can follow the run-list check and
  // stay out of the initial route-compilation burst without losing an event.
  useEffect(() => {
    const initial = setTimeout(() => void reload(), 650);
