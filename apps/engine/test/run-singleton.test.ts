...
const worktree = () => fs.mkdtempSync(path.join(os.tmpdir(), "telar-run-tree-"));
...
test("readiness is only claimed when the URL was silent before this run started", async () => {
  let answers = false;
  const manager = new RunManager({ probe: async () => ({ answered: answers, serving: answers }), readyPollMs: 20 });
...
test("a URL that was ALREADY answering can never make this run ready", async () => {
  ...
  const manager = new RunManager({ probe: async () => ({ answered: true, serving: true }), readyPollMs: 20 });
