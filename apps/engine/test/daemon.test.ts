...
test("daemon is authenticated, loopback-only, and discovers a typed engine client", async () => {
  ...
  const unauthenticated = await fetch(`http://127.0.0.1:${daemon.discovery.port}/v1/health`);
...
test("the API rejects an unregistered worker and then durably schedules a claimable turn", async () => {
  ...
  await expect(client.submitTurn(session.session.id, { runId: "run_one", text: "Hello" })).rejects.toMatchObject({
    code: "worker_unavailable",
    status: 503,
  } satisfies Partial<EngineClientError>);
  await client.registerWorker("worker_one");
  const accepted = await client.submitTurn(session.session.id, { runId: "run_one", text: "Hello" });
  expect(accepted).toMatchObject({ replayed: false, turn: { state: "queued" }, execution: { code: "scheduled" } });
  const replay = await client.submitTurn(session.session.id, { runId: "run_one", text: "Hello" });
...
test("lease expiry is pruned without another worker control request", async () => {
  ...
  await client.createSession({ id: "session_one", projectId: "project_one" });
  await client.registerWorker("worker_one");
  await client.submitTurn("session_one", { runId: "claim_me", text: "Hello" });
