...
const root = (): string => {
  const directory = knownClaudeDefault(fs.mkdtempSync(path.join(os.tmpdir(), "telar-plugin-wire-")));
...
test("the gate decides on BOTH sides, and an ungated daemon has no door at all", async () => {
  ...
  // BOTH migrated toolkits are always registered — they are shipped features,
  // and a project that has not enabled one is refused at its own gate rather
  // than by leaving it unregistered. What the env gate decides is whether the
  // proof plugin joins them.
  expect(health.plugins?.map((status) => status.meta.id)).toEqual(["latex", "data-science"]);
