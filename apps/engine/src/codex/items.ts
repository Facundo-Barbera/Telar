...
function mcpToolApproval(
  ...
  /**
   * CLASSIFIED THE SAME WAY CLAUDE'S IS, and this used to be a real product bug.
   *
   ...
   * the same tool. The engine's own classifier is the single authority for both
   * wires; the host installs the plugin half of it at startup, so a plugin's
   * reads are ratified once and answered identically wherever the call came
   * from.
   */
