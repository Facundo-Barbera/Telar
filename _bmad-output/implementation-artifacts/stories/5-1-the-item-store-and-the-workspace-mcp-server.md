...
| --- | --- | --- |
| B1 — `updateItem` moves the id in `lanes.yaml`, packet first | delete the `writeLaneRows` call | **3 tests red** (both-files, no-lane-exists, torn-move) |
| B5 — `readLanes` is tolerant per ROW | restore `WorkspaceLane.array().safeParse` all-or-nothing | **3 tests red** |
