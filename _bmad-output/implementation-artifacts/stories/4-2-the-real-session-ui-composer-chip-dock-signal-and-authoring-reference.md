...
**Round 1, `bmad-code-review`, 2026-07-27.** Diff reviewed: `e093a98..3a85c6d` (the single commit that implements this story) — 32 files, +6711/−79; code only, excluding `_bmad-output/`, 29 files, +5340/−75. Three parallel layers (Blind Hunter, Edge Case Hunter, Acceptance Auditor) produced **35 findings**; **all 35** were then put to an independent adversarial verifier instructed to refute them. **17 survived, 18 were refuted.** The 17 merge to the items below. Every number in this section was re-derived from the tree at the moment it was written, per §0.

- [x] **[Review][Decision] A run that is already terminal when the page mounts renders `0 done` — a wrong number stated as fact, which is AC11's own prohibition** [`apps/web/lib/use-ultra-runs.ts`, `useUltraRuns` — `liveIds` gates both the `EventSource` effect and the agent-index effect] — `liveIds` is `rows.filter(r => r.state === "running")`, so a run that settled before this mount gets **no** stream and **no** `/api/ultra/[id]/agents` call. `streams` and `indexes` have no other writer, so `runSnapshot(manifest, [], [], runId)` is called with empty events. Executed against the real module with a `done` manifest carrying `meta.phases: ["Scan","Rank"]`: `agentsDone: 0`, `phases: []`, `narrator: []`, `progress: {seen: 0, declared: 2}`. `ultra-anchor.tsx` renders `{run.agentsDone} done` in **both** forms (one site), so the persisted anchor of a completed five-agent run reads **`0 done`** after any reload, its sliver reads **0 of 2** for a run that completed both phases, and `ultra-rail.tsx` shows `No narration yet.` This is the mirror of the failure hard rule 3 fences: not inventing data, but **reporting real data as zero**. Confirmed independently by two review layers and by direct execution. **Why this needs a decision and not a patch:** three fixes are viable and they are not equivalent — (a) render nothing rather than `0` when no stream was observed (cheap, honest, but retreats from AC1's "agents done"); (b) open a one-shot events + agent-index read for terminal runs too (satisfies AC1, costs a fetch per historical run); (c) persist `agentsDone`/phase counts on the manifest (satisfies AC1 everywhere, but is a `storage.ts` schema change this story's §3 fences out). Pick one.

- [x] **[Review][Patch] Every `EventSource` is closed and re-opened on any live-run set change, replaying the whole journal into an accumulator that is never reset — the narrator duplicates its history** [`apps/web/lib/use-ultra-runs.ts`, the "ONE STREAM PER LIVE RUN" `useEffect` (deps `[liveIds, reload]`), its cleanup `for (const [, es] of open) es.close(); open.clear();`] — React runs that cleanup on **every** dep change, so a second run launching (or one of N live runs settling) tears down streams that are still wanted; the body then finds `open` empty and re-opens them. `app/api/ultra/[id]/events/route.ts` declares `let nextLine = 0` inside `start(controller)` — per connection — and its own header says it sends "every event from line 0". The `ev` handler appends (`events: [...prior.events, ev]`) and nothing ever resets `streams`. `narratorLines` is a bare push loop with no dedupe: verified by execution, the same three-event journal delivered twice yields `["scanning the tree","scanning the tree"]`. `agentRows` (by ordinal), `phaseGroups` (by title) and `progressFraction` (a `Set`) are immune; the narrator is not, and the `events` array grows without bound across reconnects. The trigger is exactly AC4's headline concurrent-runs case. Found by two layers independently, both confirmed at high confidence. **Same root cause:** the incremental "close streams no longer live" loop above it (`if (!wanted.has(runId))`) is unreachable dead code, because `open` is always empty at the top of every effect run.

- [x] **[Review][Patch] An unresolved `runId` renders as a live `running` run with an enabled Stop button, on every first paint** [`apps/web/lib/ultra-runs.ts`, `runSnapshot`'s `state: manifest?.state ?? "running"` reached through `spliceRunAnchors`' `runs.get(runId) ?? pending?.(runId)`] — verified by execution: `runSnapshot(null, [], [], "u-finished-days-ago")` returns `{state: "running", name: "u-finished-days-ago", pending: true}`, and `ultra-anchor.tsx`'s `canStop = run.state === "running"` therefore enables Stop. `session-view.tsx` seeds `messages` synchronously from `initialChat` while `useUltraRuns` starts `rows = []` and only fetches in an effect, so **every** persisted ultra launch in a reloaded transcript paints as "Launching…" with a spinner and the raw runId as its name until the list poll answers — and paints that way permanently if `GET /api/ultra` takes the `catch` this story added, which the dev proof measured firing against real data. D8 is right that a launch is a fact the moment a runId exists; the defect is that `pending` borrows `running` rather than being its own state.

- [x] **[Review][Patch] The Ultra chip renders on the escalation surface, where it is a placebo control — AC6 proof 6 is not satisfied** [`apps/web/components/session/session-view.tsx`, the `{provider !== "codex" && (<Button … aria-pressed={ultraArm.armed}>…Ultra</Button>)}` in `PromptInputTools`] — AC6 proof 6 says the chip is "gated off the escalation surface … the absence is enforced by the type". It is gated on `provider !== "codex"` and on nothing else. `discuss-escalation.tsx` mounts `<SessionView escalation embedded …/>` and `provider` defaults to `"claude"`, so the chip renders there, arms, and puts `ultra: true` on the same body literal that carries `role: "escalation"`. `resolveSessionProfile`'s escalation branch builds `escalationAppendix`, whose signature has neither `ultraAnnotated` nor `provider`, and `buildEscalationProfile` hard-denies all three `ULTRA_AUTO_TOOLS` — so the flag is discarded entirely. The type does enforce the absence of the *appendix*; it does not enforce the absence of the *control*. One term (`&& !escalation`) closes it.

- [x] **[Review][Patch] `settled` is never reconciled against the run's terminal state — a stopped or failed run's in-flight ordinals shimmer as live forever** [`apps/web/lib/ultra-runs.ts`, `agentRows`' `settled` field; rendered by `AgentRowView` in `apps/web/components/session/ultra-rail.tsx`] — `settled: true` is set only in the `e.type === "agent"` arm, and `runSnapshot` never crosses it with `run.state`. An ordinal that emitted `agent-start` and was then aborted — by a user Stop, or by `getUltraManifest`'s on-read `running`→`stopped` rewrite (AD-15, §5.6-T14) — keeps `settled: false` permanently, so `AgentRowView` paints the live dot and an infinitely-animating `<Shimmer>` inside a card whose header reads `stopped`. `ui-contract.md` §3 calls that snippet the **live** one; an animation that never ends on a run that ended is the placebo hard rule 3 forbids. The as-built gallery reference freezes exactly this case; production does not.

- [x] **[Review][Patch] The dock signal auto-docks the session the user is currently looking at** [`apps/web/components/common/ultra-dock-signal.tsx`, `poll`'s `if (!seeded.current.has(sessionId)) { seeded.current.add(sessionId); autoDock({…}) }`] — none of the six skips consults the route, and the file has no `usePathname`/`visibilityState`/`hasFocus`. `session-view.tsx`'s `clearAutoDock(sessionId)` effect has stable deps and fires only at mount, so a user sitting on `/projects/x/sessions/abc` while its run is `running` gets a dock head for the page they are already on within ≤8s, and `dock.tsx` mounts a second `SessionRuntimeHost` for it. `loom-notifications.tsx` — the precedent §5.5-D13 names — carries `document.visibilityState === "hidden" || !document.hasFocus()` for exactly this reason ("never notify about a loom the user is actively watching"). Found by two layers; a third verifier narrowed the harm but not the mechanism.

- [x] **[Review][Patch] The anchor grows once *while running*, when the phase sliver first appears — a second height change AC2 does not permit, and `anchorForm` cannot see it** [`apps/web/components/session/ultra-anchor.tsx`, the `{run.progress && (<div className="h-px …" />)}` inside the `{form === "running" && (…)}` block] — `run.progress` is `undefined` for D8's pending payload (`progressFraction` short-circuits on a null manifest) and becomes defined when the manifest lands, so for any script declaring `meta.phases` the running anchor gains `gap-1` + `h-px` = 5px mid-stream. AC2 permits exactly one height change and it is the terminal collapse; the file's own comment says "There is no conditional row inside it". It matters mechanically for the reason AC2 proof 4 gives: `StickToBottom` is configured `resize="smooth"`, so anything that grows mid-stream animates the transcript. **And AC2's executable stand-in is structurally blind to it** — `anchorForm(run)` reads `state` alone, so this is a guard that cannot fail against maxim 3.

- [x] **[Review][Patch] Five sentences in this story's own record claim what the tree does not support** [`_bmad-output/implementation-artifacts/stories/4-2-*.md` §9/§10, and `apps/web/lib/ultra-runs.test.ts`] — the defect class §0 declares binding on this file, re-derived here by command:
  1. `apps/web/lib/ultra-mcp.ts` is disclosed twice as **"net −26 — the description shrank"** (§9 §6 "Inside Owns", and §10's MODIFIED row). `git diff --numstat e093a98 HEAD` → `30 25`; the file went **243 → 248 lines, net +5**. It grew. The substantive claim underneath is true and is worth keeping — `ULTRA_TOOL_DESCRIPTION` itself went 28 → 11 lines, 3335 → 1714 chars — but the file did not shrink, because the cross-reference comments added more than the description dropped. The `55` in the Δ column is correct (30+25, the convention all 20 rows use).
  2. §10's NEW table gives `apps/web/components/session/ultra-rail.tsx` as **530**; it is **535** (`wc -l`, `git show 3a85c6d:…`, and the commit's own diffstat all agree). The header **"NEW (11 files, 3749 lines)"** is the sum of the table's own rows carrying that error and should read **3754**. Most likely the count was taken before the NUL-byte repair §10 item 10 records, which is precisely the "measure at the moment you write it" rule.
  3. §10's **"Total diff: 31 files, 1737 insertions, 80 deletions"** — the insertions and deletions are exactly right and reproduce from the command the sentence cites (20 modified tracked files, +1631/−79, plus the dirty `orchestrator-run-log.md` at +106/−1 = **1737 / 80**). The **file count is 21, not 31.** Two of three figures being exactly right is what makes the third a transcription slip rather than a fabrication.
  4. §9 §4's **"The real `~/.telar` was never read or written at any point in this story"** is false as to *read*. `packages/core/test/track-d-prove-run.test.ts` does `fs.readdirSync(os.homedir()/".telar")` plus a `statSync` per entry, at module scope and again in L7 — which is the only way the Debug Log's own `[track-d] L7 real ~/.telar unchanged (2 entries)` line could exist. Never **written** is true and is the property that matters; the sentence overclaims by one verb.
  5. `apps/web/lib/ultra-runs.test.ts`'s comment **"`GET /api/ultra` has no callers today but it is a published shape"** is falsified by the same commit, which adds two: `ultra-dock-signal.tsx`'s unfiltered `fetch("/api/ultra")` on an 8s app-wide interval, and `use-ultra-runs.ts`'s `fetch("/api/ultra?sessionId=…")`. The route's own header names "the rail and the dock" as consumers. Inherited from §5.5-D5, which contradicts §2 AC7 proof 1 in the same file.

- [x] **[Review][Patch] The dock head's `initial` splits an astral first character into a lone surrogate** [`apps/web/components/common/ultra-dock-signal.tsx`, the `initial:` expression in the `autoDock({…})` call] — `(title.trim()[0] ?? chat.project.trim()[0] ?? "·").toUpperCase()` indexes UTF-16 code units, so a title beginning with an emoji yields half a surrogate pair and the head renders tofu. Titles come from the user's own first message, so this is ordinary input. §5.6-T19 item 9 states the repo rule ("truncate by code point … `Array.from(...)`; `.slice()` splits astral characters") and this story followed it in `agents/route.ts`; this one site missed it.

- [x] **[Review][Patch] Two cosmetic leftovers in new files** [`apps/web/components/session/ultra-rail.tsx` `ScriptTab`; `apps/web/components/session/ultra-anchor.tsx`] — `ScriptTab` omits the `cancelled` re-check after its second suspension point (`await r.json()`), unlike `AgentTranscript` and `Result` in the same file; because `RunCard` is keyed by `runId` no wrong-run script can be written, so this is a style inconsistency rather than a latent bug. And `ultra-anchor.tsx` carries `{view.live && null}`, which renders nothing in either branch — dead scaffolding.

- [x] **[Review][Defer] Two raw NUL bytes sit in `apps/web/lib/server/git-tab.ts`, untouched by this story** [`apps/web/lib/server/git-tab.ts`, offsets 2261 and 26024 — `const NUL = "<NUL>"` and a `.replace(/<NUL>/g, "")` regex literal] — deferred, pre-existing (commit `d87ad57`, outside this story's write set). Found while auditing this story's own NUL close-out scan. It reproduces the exact harm §9 describes: `grep 'const' apps/web/lib/server/git-tab.ts` prints `Binary file … matches` instead of its 190 matching lines. Both uses are legitimate `git … -z` separator handling and both are valid TypeScript; the fix is to write them as the escape rather than the byte, which is the rule §9 arrived at. **This means the run's "fourth occurrence" tally undercounts the tree.** Owner: whichever story next opens `git-tab.ts`.

- [x] **[Review][Defer] `GET /api/ultra/[id]/agents` re-reads and re-parses every agent transcript in full, plus the whole events journal, on every 4s poll** [`apps/web/app/api/ultra/[id]/agents/route.ts`, `GET` — `readUltraEvents(id, 0)` and the `listUltraAgentOrdinals(id).map(readUltraAgentTranscript)` loop] — deferred, not user-visible today. `readUltraAgentTranscript` has no offset, cap or cache, so the handler's cost is O(all bytes every agent has ever streamed) while its answer is a 200-code-point snippet plus a flag per ordinal. A verifier measured the realistic worst case at 16–30 ms per 4 s (~0.5% duty cycle) and noted the pre-existing SSE tail already does a full read of the same journal every 400 ms, so this is not the dominant cost and not novel. Real but not actionable now; a byte-offset or last-N-lines read would make it O(new bytes). Owner: whichever story first runs an Ultra script at a scale where it bites.

**Round 1 fix pass, 2026-07-27 — all ten open items closed, with the gate re-measured rather than carried.** Every figure below came from a command run in that pass; nothing was copied from the review's own report, and where the two disagree the disagreement is stated rather than smoothed over. Full record in Debug Log **§11**.

**The `[Review][Decision]` item was decided as (b), the reviewer's recommendation, plus half of (a) — and the "plus" is deliberate.** (b) is the fix: `use-ultra-runs.ts` now opens the events stream and the `agents/` index for **terminal** runs too, exactly once each, so a run that settled before the page mounted is projected from its own journal and `5 done` is `5 done`. But (b) has a window and a failure mode — the read takes a moment, and it can fail — and `0 done` during either is the same wrong number, just rarer. So `RunSnapshot.agentsDone` is now **optional and absent** until the journal has actually been read, `runSnapshot`'s events parameter takes `null` (meaning "not read") as distinct from `[]` (meaning "read, and empty"), and the anchor renders nothing where it cannot source a figure. That is the file's existing doctrine — `agentsTotal` typed `undefined`, `progress` absent rather than zero — extended from *what the engine cannot produce* to *what this reader has not read*. (c) was not taken: it is a `storage.ts` schema change §3 fences out, and it would not have helped a single manifest already on disk.

**Two deviations from the review's prescribed fixes, both argued rather than silently taken.**

1. **SF-4 is a ROUTE guard, not the focus guard the finding prescribed.** `loom-notifications.tsx`'s `visibilityState === "hidden" || !document.hasFocus()` is right for a *notification*, whose premise is that the user is away. Here it would break AC7's own Given — "the user is on **another page** with a run live" describes a **focused** tab, so a focus guard would suppress the dock head in precisely the case the AC exists for. What must be skipped is one **session**, not one tab, and that is a fact about the route: `isSessionRoute(pathname, sessionId)`, pure and tested six ways. Read from `window.location` inside the poll rather than through `usePathname()`, because this component renders nothing and `usePathname` in a root-layout component carries the prerender/`Suspense` constraint under `cacheComponents` that Next's own reference states — the same class of constraint §5.5-G5 weighed for `useSearchParams`.
2. **SF-5's widened AC2 helper is TWO functions, not one.** `anchorShape` carries the height decisions (`form`, `detail`, `sliver`, `errorRow`) and is asserted constant across a running run whose `progress` appears mid-stream — the exact case `anchorForm` was blind to. `anchorControls` carries `canStop`/`canResume`, which sit in a **non-wrapping flex row** and therefore change the anchor's width and never its height. Folding them together made AC2's "the shape is constant while running" fail on the pending→resolved transition, on a difference AC2 does not forbid — and a proof that has to be told which of its own fields to ignore is prose again.

**One figure in the review's own report did not reproduce, and it is one it flagged as true.** B3 item 1 keeps the substantive claim that `ULTRA_TOOL_DESCRIPTION` shrank, and gives it as "28 → 11 lines, 3335 → 1714 chars". Re-measured this pass with a scanner that honours the **escaped backticks** inside the `e093a98` literal (a naive `indexOf("\`")` stops at the first one and reports 22 lines / 3006 chars, which is how this is easy to get wrong): **28 → 11 lines** reproduces exactly; the character counts are **3331 → 1710** for the raw source between the delimiters, or **3321 → 1710** for the string the model actually receives. The review's pair is +4 on both sides, which is the two backticks plus `;` plus the newline — a defensible slice, but not the one named. The figure below is the string the model receives, and it says so.

**Dismissed as noise: 18 of 35.** The largest refutations, recorded because each is a claim a future reviewer will re-raise: the arm flag surviving a provider switch is inert (the Codex fork has no `systemPrompt` field at all — `CodexRunOptions` never carries the appendix, and `route.ts` says so in its own words); `progressFraction` not intersecting observed titles with declared ones is the specified design, not a defect; the app-wide 8s poller is the shipped house pattern (`AppSidebar` fires four unconditional fetches every 10s from the same layout) and its deviation is disclosed in the file header and in `deferred-work.md`; the authoring reference's `isolation` bullet is inert but shipped **byte-identical** in `ULTRA_TOOL_DESCRIPTION` since `84a6e7c`, so this commit changed no model-facing exposure; and the AC9 scan's four-file enumeration is exactly what AC9 proof 2 prescribes.
...
**`TELAR_HOME`, confirmed before starting rather than assumed.** `apps/web/package.json`'s `dev` script is `TELAR_HOME="${TELAR_HOME:-$HOME/.telar-dev}" next dev`. The server resolved **`~/.telar-dev`**. The real `~/.telar` was never **written** at any point in this story — verified by content hash across a full run of both workspaces, not inferred. *(Corrected in the round-1 fix pass: the original sentence said "never read or written", and the read half was false. `packages/core/test/track-d-prove-run.test.ts:85` builds `path.join(os.homedir(), ".telar")` and does a `readdirSync` plus a `statSync` per entry — at module scope and again in L7 — which is the only way the Debug Log's own `[track-d] L7 real ~/.telar unchanged (2 entries)` line could exist. **Read, deliberately and read-only; never written**, which is the property that matters and the one INV-7 exists for.)*
...
**Inside Owns** (disclosed for completeness): `apps/web/lib/ultra-mcp.ts` (**+30/−25, net +5 — the FILE grew while the DESCRIPTION shrank**), `packages/core/src/ultra/executor.ts` (+60), `packages/core/src/ultra/storage.ts` (+36), and every new `lib/ultra-*`/`app/api/ultra/**` file.

*(Corrected in the round-1 fix pass; the original read "+15/−41 net −26 — the description shrank", and only the last four words survived contact with the tree. `git diff --numstat e093a98 -- apps/web/lib/ultra-mcp.ts` → **`30 25`**, and the file went **243 → 248 lines**. The claim underneath is real and is why the slip was plausible: `ULTRA_TOOL_DESCRIPTION` itself went **28 lines → 11**, and **3321 → 1710 characters** of string the model actually receives — measured this pass by scanning the template literal from its opening backtick while honouring escaped backticks, of which the `e093a98` version has several. The file still grew, because the cross-reference comments AC8 asked for added more than the description dropped.)*
...
The repair does not re-use a separator at all: the key is now `` `phase:${group.title}` ``, which also fixes a latent collision (a real phase titled `unphased` would have shared the fallback key). Re-scanned after the fix: **33 files, every one `0`**.

**CORRECTED IN THE ROUND-1 FIX PASS — the sentence that used to end this item was exactly backwards, and it was a guard that cannot fail sitting inside the note about guards that cannot fail.** It read: *"`grep`/`grep -a` counts are now identical on the repaired file … that parity check, not the byte count, is what demonstrates the failure mode is gone."* **`grep -c` and `grep -ac` return IDENTICAL counts on a NUL-poisoned file** — `-c` bypasses grep's binary-suppression path entirely and simply counts matching lines in both modes. Measured this pass, in both directions, with the NUL on the matching line and on a different line:

```
fixture                       NULs   grep -c   grep -ac   grep (line output)
clean                            0         2          2   const a = 1;
NUL inside the matching line     1         2          2   Binary file … matches
NUL on another line              1         2          2   Binary file … matches
```

So parity proves nothing; it holds whether or not the byte is there. **The observable difference lives in LINE OUTPUT, not in counts** — plain `grep` prints `Binary file … matches` where `grep -a` prints the real matches — and the thing that actually demonstrates the repair is **the byte count the sentence demoted**: `tr -dc '\0' < FILE | wc -c` → 0, which was itself given a two-direction discriminator (a 1-NUL fixture returns 1, a 3-NUL fixture returns 3). `file(1)` reporting *"Java source, Unicode text, UTF-8 text"* is real corroboration and is kept; the ranking is what was wrong.

**The lesson, restated because this run keeps paying for it:** the scan is only worth what its own mechanics are worth. A byte scan that cannot fail is worse than no byte scan — and so is the sentence that promotes a *second* check above it without checking that one either.

### 11. The round-1 fix pass — what changed, and every figure re-measured

**Ten open review items, all ten closed. No new file, no new fence, no new dependency, no `bunfig.toml` edit, no `KNOWN_VIOLATIONS` entry, no `sprint-status.yaml` edit.** **Seven** code and test files changed — `lib/ultra-runs.ts`, `lib/use-ultra-runs.ts`, `lib/ultra-runs.test.ts`, `components/session/ultra-anchor.tsx`, `components/session/ultra-rail.tsx`, `components/common/ultra-dock-signal.tsx`, `components/session/session-view.tsx` — every one already in this story's write set, and six of the seven are files this story created. Plus two record files: this one, and `deferred-work.md`.

**The gate, re-run in this pass and not carried:**

| Check | Before this pass | After |
| --- | --- | --- |
| `bun test packages/core` | 1660 / 0 / 9347 / 110 | **1660 pass / 0 fail / 9347 expect() / 110 files** — unchanged |
| `bun test apps/web` | 610 / 0 / 2366 / 22 | **637 pass / 0 fail / 2469 expect() / 22 files** (+27 tests, +103 assertions) |
| `bunx tsc --noEmit` `packages/core` | clean | **exit 0, no output** |
| `bunx tsc --noEmit` `apps/web` | clean | **exit 0, no output** |
| `bun run lint` `apps/web` | 163 problems | **136 errors + 27 warnings = 163**, across 45 files — **zero added** |
| `bun run lint` `packages/core` | no such script | **still no `scripts` key** — absence recorded, not created |
| Track D prove-run | 7 legs | **7 pass / 0 fail / 41 expect()**, 1653 filtered out |

**Zero lint problems added, and that is measured rather than asserted — after the first attempt at measuring it was itself wrong.** Of the seven files this pass touched, **six appear nowhere in the lint report at all**. `session-view.tsx` appears with **nine** problems, at **93:10, 381:19, 977:10, 1038:10, 1322:65, 1678:6, 1884:5, 1962:19 and 2021:17** — while this pass's only hunks in that file are at **2148–2159** and **3206–3230**, per `git diff -U0`. All nine predate this story; none is within 700 lines of anything this pass wrote.

*(This paragraph first said **two**, naming only 93:10 and 381:19, and an adversarial pass over this fix round caught it. The cause is worth recording because it is the same shape as §10 item 10's: `react-hooks/set-state-in-effect` prints a multi-line code frame that BEGINS WITH A SECOND `path:line:col` HEADER, so a parser that treats any leading-`/` line as a file boundary silently attributes the following seven problems to a phantom file. The corrected parser self-validates — it totals **163 problems across 45 files**, which is exactly the footer eslint prints (`✖ 163 problems (136 errors, 27 warnings)`) — and the first one did not, which is what a total is for.)*

**`~/.telar` untouched, verified empirically.** Fingerprinted (relative path, byte size and SHA-256 per file), then a **full** `bun test` of both workspaces, then re-fingerprinted: **2 files, identical digest, byte-for-byte unchanged** (`accounts.json` 296 B, `usage.ndjson` 201 B).

**The NUL scan, run twice.** All **10** changed files read `0` by `tr -dc '\0' | wc -c`. And the review's deferred item was **acted on rather than believed**: a repo-wide scan over `git ls-tree -r --name-only HEAD` — the scan every previous pass in this run scoped to its own touched files — finds exactly three files with NUL bytes, and `apps/web/lib/server/git-tab.ts` (**2 bytes**) is the only source file among them; the other two are `apps/desktop/build/icon.icns` (3) and `apps/web/app/favicon.ico` (27), which are genuine binaries. The deferred finding reproduces. **The run's occurrence tally still counts occurrences CAUGHT, not bytes present**, and now there is a one-command scan that counts the latter.

**Ten revert probes, each restored.** Every new pin was proved load-bearing by breaking the thing it guards and watching it go red, then restoring the file from a copy:

| Probe — the fix, reverted | Turns red |
| --- | --- |
| `canStop` drops `&& !run.pending` | B2's *"the pending payload offers NO Stop"* |
| `agentsDone` stated unconditionally | both B1 pins, including the `[]`-vs-`null` discriminator |
| `live` becomes `!settled` again | two SF-3 pins |
| `wantedStreams` wants only live runs | SF-1's *"an UNDRAINED terminal run is wanted"* |
| the sliver goes back to `{run.progress && (` | the SF-5 static pin |
| the chip loses `&& !escalation` | the SF-2 pin |
| the dock stops consulting the route | the SF-4 / NH-1 pin |
| `allIds` leaves the stream effect's key | the SF-1 hook pin |
| the pending factory passes `[]` instead of `null` | the SF-2 static pin, which also carries the pending-factory line |
| `hydrated.current.delete(runId)` removed from the stream-open loop | the SF-1 hook pin |

**AN ADVERSARIAL PASS WAS RUN OVER THIS FIX ROUND, AND IT FOUND A REGRESSION THE FIX ROUND HAD INTRODUCED.** Four independent fresh-context auditors — one per lens (every figure re-derived; the code's own correctness; whether each of the ten items is genuinely closed; what the pass broke or weakened) — then one skeptic per finding instructed to refute it, default verdict REFUTED. **17 raised, 8 refuted, 9 survived**, and every one of the nine is fixed above rather than argued away. The one that mattered:

> **A resumed run could lose its journal permanently.** `use-ultra-runs.ts` — `hydrated` was ADD-ONLY, which encodes the claim "a run drained once never needs reading again". **A resume falsifies it.** `resumeUltraRun` re-runs under the EXISTING runId, so one runId goes terminal → running → terminal; the second connection resets `drained` to false; and the list poll is 4 s while the SSE tail is 400 ms, so there is a real window in which the run leaves the live set *while still marked hydrated from its first pass* — at which point the reconciler closes a stream that has not delivered `end`, `drained` stays false forever, and the projection reads `journal = null` for that run for the life of the page. **The B1 fix would have silently un-fixed itself for exactly the runs AC5's Resume affordance exists to produce.** The auditor reproduced it by driving the real hook against a fake `EventSource`, not by reading. **Fix:** opening a stream deletes the run from `hydrated`, so `hydrated` means "the CURRENT connection drained". Pinned two ways — a pure test that walks the whole resume sequence with its own anti-vacuity half (the same step with a stale `hydrated` DOES close the stream), and a static pin that the delete is in the open loop.

The other eight were record defects in this pass's own new prose — the lint per-file count above, the `focusRunId` line number, `deferred-work.md`'s before-value and its hunk range, the Completion Notes' stale `apps/web 610/0`, and a `git grep` citation inside the very comment B3 item 5 fixes whose pattern (`fetch("/api/ultra`) matched nothing at `e093a98` because every call in this repo but one is a **backtick template literal**. **That last one is the irony this run keeps re-paying for, caught this time before it shipped:** a sentence citing a command, inside the fix for sentences that cite commands, whose command returns nothing. It now cites `git grep -nE 'fetch\(.?/api/ultra' e093a98 -- apps/web`, which returns the one baseline line it claims.

**What each item became, in the tree:**

1. **B1 (Decision) → option (b), plus an honesty guard.** `use-ultra-runs.ts` gained `allIds` and a `hydrated` ref; the stream effect now wants every live run **plus** every run it has not drained to `end`, and the `agents/` index is pulled once for every run and polled only for live ones. The route already replays from line 0 and sends `end` for a terminal run, so one open **is** a one-shot journal read. Separately, `runSnapshot`'s events parameter now accepts `null` — "not read" — distinct from `[]`, and `RunSnapshot.agentsDone` is **absent** rather than `0` when neither the journal nor the agent index has answered. **That distinction caught a second site the review did not name and (b) alone would not have fixed:** `session-view.tsx`'s D8 pending factory passed `[]`, and the pending window is exactly the first paint of a reloaded session — so **every** persisted launch in the transcript, including one that settled five agents last week, read `0 done` for the width of the list poll. It now passes `null`. `ultra-dock-signal.tsx`'s `runSnapshot(m, null)` is the same correction on the same grounds. Cost stated rather than hidden: one stream and one `agents/` request per run per page load, bounded by the session's own run count.
2. **B2 → `anchorControls(run)`.** `canStop` is `run.state === "running" && !run.pending`. The badge says `launching` rather than `running` while pending, because `pending` is the only thing that reader actually knows.
3. **SF-1 → `wantedStreams` + `reconcileStreams`, both pure and both tested.** The reconciling effect's cleanup no longer closes anything; the wipe moved to its own mount-only effect. The formerly-dead incremental loop is now the live path. And the accumulator resets on the `open` event rather than never — which also covers an `EventSource` reconnecting after a transport error, a replay the original code would have appended a second time.
4. **SF-2 → one term.** `{provider !== "codex" && !escalation && (`.
5. **SF-3 → `AgentRow.live`, decided in the projection.** `agentRows` takes the owning run's state; an unsettled ordinal on a run that has ended is neither settled nor live. `agentsDone` still counts settlement only — marking an aborted agent settled to stop the shimmer would have traded one false statement for another.
6. **SF-4 → `isSessionRoute(pathname, sessionId)`.** See the two argued deviations in §4's fix-pass note.
7. **SF-5 → `anchorShape(run)`.** The sliver's row is reserved in the running form and its ink is conditional (`opacity-0`), so AC2 keeps its one permitted height change and AC11 proof 4 keeps "no sliver at all". The AC2 proof now drives five successive snapshots across which `progress` really does appear, and asserts the shape object is identical at all five.
8. **NH-1 → `Array.from(title.trim())[0]`.**
9. **NH-2 → the `cancelled` re-check in `ScriptTab`, and `{view.live && null}` deleted.** The renderer now takes `(payload)` alone; a function of fewer parameters is assignable to `ItemRenderer`.
10. **B3 → five sentences plus the test comment**, each re-derived by command in this pass. Corrections are in place above with the measurement beside them, never as a silent overwrite. A sixth was corrected on the same grounds although it was filed only as a scrutiny point: §10 item 10's claim that `grep`/`grep -a` **count** parity demonstrates the NUL failure mode is gone. It does not — the counts are identical either way, measured here in both directions.

**What this pass did NOT prove, said plainly.** The environment is unchanged: `apps/web/node_modules/@anthropic-ai/` still holds `claude-agent-sdk` alone, so **no model turn is possible and no browser observation was taken**. Every fix that ends in a rendered pixel — the reserved sliver's constant height, the absent `N done`, the disabled Stop, the missing chip on the escalation surface, the dock head that no longer appears for the page you are on — is proved by a pure function plus, where the decision is a line of JSX, a static pin that the component calls that function. **That is the same honest ceiling §6.2 set for the implementation, and this pass did not raise it.** In particular: SF-4's guard is proved as a route predicate and as a pin that the poller consults it; nobody watched a dock head fail to appear.

### Completion Notes

**Status: every AC has an implementation and a proof, and the two things that could not be PROVED are named rather than implied.** The full gate is green in both workspaces (at `3a85c6d`: `packages/core` 1660/0, `apps/web` 610/0, `tsc` clean in both, `apps/web` lint 163 problems against a 164 baseline with **zero added**; **after the round-1 fix pass, `apps/web` is 637/0** and the rest is unmoved — Debug Log §11 carries the whole re-measured table), and the Track D prove-run passes all seven legs including L2, the one that proves the executor change landed rather than merely compiled.
...
- **TWO UNDER-COUNTS IN THIS VERY LIST, added in the round-1 fix pass because "every one disclosed" is a claim and the review found two it did not cover.** Neither is a hidden edit — both are edits disclosed elsewhere and missing from an enumeration here — and both were re-measured by command rather than accepted on the review's word.
  - **`session-view.tsx` carries a TWELFTH expression, not eleven: `focusRunId`,** in three places — `:648` (outer props type), `:699` (the destructure), `:713` (inner props type) — plus the effect at `:2183` that §5.5-D1 item 9 does name. It exists because §5.5-G5 **shape (b)** was taken, threading the `?run=` param as a prop from `app/projects/[name]/sessions/[id]/page.tsx` instead of reading it with `useSearchParams`. The **choice** is disclosed (that page is the ninth out-of-Owns row, with the measured prerender reason); the **prop plumbing it costs** was not in §9 §6b's table. Same class as the eleventh, which this story did disclose — so: **it is twelve, and this line says twelve.**
  - **`dock-provider.tsx`'s `Runtime` gained TWO fields, not one.** §0.2's write-set row says "one"; the diff adds `ultraSummary?: string` and `ultraFocusRunId?: string` — `git diff e093a98 -- apps/web/components/dock/dock-provider.tsx` shows both. Disclosed in `deferred-work.md`'s dock item and absent from this list until now.
...
**RE-MEASURED IN THE ROUND-1 FIX PASS, and the BASIS IS NAMED because the old one no longer exists.** The original tables were taken from `git diff --stat HEAD` against an uncommitted working tree, where the eleven new files were still untracked and therefore uncounted. They are now committed, so this table is taken from **`git diff --numstat e093a98`** against the tree at the close of the fix round — one range, one command, every file counted the same way, and it keeps reproducing as `git diff --numstat e093a98 HEAD` once this round is committed. NEW rows are `wc -l`; MODIFIED rows are Δ = insertions + deletions, the convention every row here uses. `_bmad-output/implementation-artifacts/orchestrator-run-log.md` shows as modified in the working tree but was **not** touched by this story and is **deliberately excluded from the commit** (it belongs to the orchestrator); it is excluded from every figure below and its own diff is stated separately.

**NEW (11 files, 4577 lines):**

| Path | lines |
| --- | --- |
| `apps/web/lib/ultra-runs.ts` | 860 |
| `apps/web/lib/ultra-runs.test.ts` | 1383 |
| `apps/web/lib/ultra-authoring.ts` | 156 |
| `apps/web/lib/ultra-authoring.test.ts` | 141 |
| `apps/web/lib/use-ultra-runs.ts` | 380 |
| `apps/web/components/session/ultra-anchor.tsx` | 331 |
| `apps/web/components/session/ultra-rail.tsx` | 551 |
| `apps/web/components/common/ultra-dock-signal.tsx` | 225 |
...
*(At `3a85c6d` — implementation only, before the fix round — the same eleven files were **3754** lines, of which `ultra-rail.tsx` was **535**. §10 shipped that header as **3749** and that row as **530**: the header was exactly the sum of the table's own rows carrying the row's error, which is the arithmetic signature of a count taken before the NUL-byte repair item 10 records and never re-taken. Both are corrected here by re-measuring rather than by patching the arithmetic.)*
...
| `apps/web/components/session/session-view.tsx` | 329 |
...
| `apps/web/lib/ultra-mcp.ts` | 55 (**+30/−25, net +5** — the FILE grew; the DESCRIPTION shrank 28 → 11 lines) |
| `apps/web/lib/ultra-mcp.test.ts` | 120 |
| `_bmad-output/implementation-artifacts/deferred-work.md` | 61 |
| `_bmad-output/implementation-artifacts/sprint-status.yaml` | 8 |

**Eight rows moved in the round-1 fix pass**, and none of them is a new file or a new fence: `ultra-runs.ts`, `ultra-runs.test.ts`, `use-ultra-runs.ts`, `ultra-anchor.tsx`, `ultra-rail.tsx` and `ultra-dock-signal.tsx` (six NEW rows, all this story's own); `session-view.tsx` (302 → **329** Δ), whose two fix-round hunks are the pending factory's `null` events argument at **2148–2159** and the chip's `&& !escalation` term with its comment at **3206–3230**, measured by `git diff -U0`; and `deferred-work.md` (**41** at `3a85c6d`, **51** once the code review appended its own findings section — which was already in the working tree when this pass began — and **61** now, having gained the fix pass's own three deferred items).

Plus this story file (§4 checkboxes and findings, §9, §10, §11, and the frontmatter `status`).
...
**Total diff: 31 files (11 new + 20 modified), 6255 insertions, 79 deletions.** Basis, spelled out because the old sentence's was not: `git diff --numstat e093a98` at the close of the round-1 fix pass, **excluding this story file and `orchestrator-run-log.md`**. Excluding all of `_bmad-output/` as well — code only — it is **29 files, 6190 insertions, 75 deletions**. The run log's own diff, kept out of the commit, is **+106/−1**.

*(The shipped sentence read "31 files, 1737 insertions, 80 deletions". **The insertions and deletions were exactly right** for the basis they were taken on — `git diff --stat HEAD` against the then-uncommitted tree, which counts the 20 modified tracked files at +1631/−79 plus the dirty run log at +106/−1 — and **the file count was 21, not 31**. Two of three figures reproducing to the digit is what makes it a transcription slip. That basis no longer exists once the eleven new files are committed, so the sentence was re-derived on a basis that survives being committed rather than being patched from 31 to 21. It is 31 again, on different arithmetic, and saying so is the point.)*
...
| 2026-07-27 | **A NUL byte was shipped into `ultra-rail.tsx` and caught by the close-out scan — the FOURTH occurrence in this run.** Offset 10544, line 249, inside `key={group.title \|\| "<NUL>unphased"}`. Worse, **the first run of that scan reported a false ALL ZERO**: it used `for f in $FILES` in zsh, which does not word-split an unquoted parameter, so it scanned one 2000-character "filename". Rewritten to iterate `git status` line by line it found the byte immediately. The repair uses no separator at all (`` `phase:${group.title}` ``), which also removes a latent key collision. All 33 touched files now read 0. *(The clause that used to end this row — "`grep`/`grep -a` counts are identical on the repaired file — that parity, not the byte count, is what shows the failure mode is gone" — was **corrected in the round-1 fix pass**: `grep -c` and `grep -ac` return identical counts on a NUL-poisoned file too, measured in both directions, so the parity is a guard that cannot fail. The byte count it demoted is the check that works. Details in Debug Log §10.)* **The lesson this run keeps re-paying for: a scan is worth only what its own mechanics are worth.** |
| 2026-07-27 | **Code review round 1 — 35 findings raised, all 35 adversarially verified, 17 survived.** Diff reviewed: `e093a98..3a85c6d`, the single commit implementing this story. Three parallel layers (Blind Hunter · Edge Case Hunter · Acceptance Auditor), then one independent skeptic per finding instructed to refute it; **18 were refuted**, which is the number worth reading, because several were confident and specific and still wrong. The gate was re-measured rather than trusted and reproduces the story's figures **exactly**: `packages/core` 1660/0/9347/110, `apps/web` 610/0/2366/22, `tsc` clean in both, `apps/web` lint 163 problems with **zero on any line this diff added** and zero in any of the 11 new files. `INV-7`, `INV-8` (13 arms), `INV-10` (6), `INV-3f` all green; `KNOWN_VIOLATIONS` still exactly one; `bunfig.toml` untouched; the real `~/.telar` byte-identical by content hash across a full run of both suites. **The two findings that matter most are both about a run the page did not watch happen:** a terminal-at-mount run projects `agentsDone: 0` and renders `0 done` as a fact (`[Review][Decision]`), and an unresolved runId renders as a live `running` run with an enabled Stop on every first paint. **And the defect class §0 declares binding on this file survived five times in §9/§10** — `ultra-mcp.ts`'s "net −26" (it is +5), `ultra-rail.tsx`'s 530 (it is 535, moving the NEW total to 3754), "31 files" (it is 21, while the insertions and deletions in the same sentence are exactly right), "the real `~/.telar` was never read" (never *written* is true; L7 reads it), and a test comment saying `GET /api/ultra` has no callers in the commit that adds two. |
| 2026-07-27 | **Code review round 1 — fix pass, attempt 1 of 2. All ten open items closed; seven files touched, all already in the write set.** Gate re-measured in the pass, not carried: `packages/core` **1660 / 0 / 9347 / 110** (unchanged), `apps/web` **637 / 0 / 2469 / 22** (was 610 / 0 / 2366 — **+27 tests, +103 assertions**), `tsc` clean in both, `apps/web` lint **163 problems (136 errors + 27 warnings) — zero added**, prove-run **7 / 0 / 41**. Real `~/.telar` re-fingerprinted around a full run of both suites: **2 files, identical SHA-256**. **Ten revert probes**, each turning exactly the pin it targets red, each restored. **The `[Review][Decision]` was decided as (b) — read a terminal run's journal once, so `5 done` is `5 done` — plus half of (a), because (b) has a window and a failure mode and `0 done` in either is the same wrong number: `agentsDone` is now ABSENT rather than `0` until the journal has been read, and `runSnapshot` takes `null` for "not read" as distinct from `[]` for "read, and empty".** Two prescribed fixes were deviated from with the argument stated rather than silently: SF-4 is a **route** guard and not the precedent's focus guard, because a focus guard would suppress the dock head in exactly the case AC7 exists for; and SF-5's widened AC2 helper is **two** functions, because the two controls change the anchor's width and not its height and a proof that must be told which of its fields to ignore is prose again. **The five B3 sentences were re-derived by command in this pass rather than patched from the review's report — and one figure the review vouched for did not reproduce:** `ULTRA_TOOL_DESCRIPTION` is 28 → 11 lines exactly as reported, but 3321 → 1710 characters of model-facing string (3331 → 1710 of raw source), not 3335 → 1714; the review's pair is +4 on both sides, which is the delimiters plus `;` plus the newline. **A sixth false sentence was corrected although it was filed only as a scrutiny point**: `grep -c` and `grep -ac` return **identical** counts on a NUL-poisoned file, measured here in both directions, so §10's claim that the parity rather than the byte count demonstrates the repair was exactly backwards — a guard that cannot fail, inside the note about guards that cannot fail. **§10's tables were re-measured on a basis that survives being committed** (`git diff --numstat e093a98`, one range, one command) rather than patched digit by digit, because the old basis — an uncommitted tree where the eleven new files were untracked and uncounted — no longer exists. **An adversarial pass was then run over the fix round itself** — four fresh-context auditors (figures · code · closure · regressions), one refuting skeptic per finding, default verdict REFUTED: **17 raised, 8 refuted, 9 survived, all 9 fixed.** The one that was not prose: **`hydrated` was ADD-ONLY, so a RESUMED run could lose its journal permanently** — a resume re-runs under the existing runId, and the 4 s list poll can see the second terminal manifest before the 400 ms SSE tail delivers `end`, so the reconciler closed a stream that had not drained and the projection read `journal = null` for that run for the life of the page, silently un-fixing B1 for exactly the runs AC5's Resume affordance produces. Opening now un-hydrates. **The other eight were this pass's own new prose, including — and this is the class the round exists to fix, committed inside the round's own record — a `git grep` citation whose pattern matched nothing because every call in the repo but one is a backtick template literal, and a `deferred-work.md` hunk range measured before the pass's own second hunk landed and never re-taken.** Both are corrected in place with the measurement beside them. |
