...
- **Compose the per-owner service-lease registry** over `SPEC-runtime-foundations` CAP-5's generalized lease primitive — this spec consumes that primitive and re-implements no stale-reclaim. Loom-owned service leases live at `TELAR_HOME/projects/<id>/looms/<loom-id>/services/<name>.json`, under the canonical per-loom root (the legacy flat `~/.telar/looms/<id>/` tree is read-only for pre-redesign looms — see `map-and-storage.md`). Generalizing `runner/lease.ts` and creating the per-chat-session tree at `TELAR_HOME/sessions/<sessionId>/` are CAP-5's, not this spec's.
