...
- **The master is a full harness session** (CC/Codex CLI, like project sessions — not an SDK-native agent): project gate skipped, `cwd` = the dedicated empty subdirectory `TELAR_HOME/workspace/home` — never the store root itself, so `lanes.yaml` and `packets/` stay outside the master's path-based write boundary — `settingSources: []`, default guardrails, MCP injected programmatically. Never use `/Users/facundo` as cwd — trust never persists there, and per-directory keying means a stable home accrues one continuous history bucket while scratch dirs fragment it. See `brownfield.md`.
