...
export const EngineHealth = z.object({
   ...
   * ignores this one, which is exactly the tolerance we want while there is no
...
export const EngineErrorCode = z.enum([
  ...
  "textgen_failed",
  /**
   * A PLUGIN FAILED, and the id is on the message. A broken plugin reads as ITS
   * failure rather than as "the engine did something", which is the difference
   * between a person knowing which switch to turn off and filing a bug.
   */
  "plugin_error",
  "internal_error",
]);
